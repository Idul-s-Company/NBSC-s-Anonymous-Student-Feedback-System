...
$where = []; $params = [];
if ($filter_status)   { $where[] = 'f.status=?';           $params[] = $filter_status; }
if ($filter_priority) { $where[] = 'f.priority=?';         $params[] = $filter_priority; }
if ($filter_category) { $where[] = 'f.category_id=?';      $params[] = $filter_category; }

$sql = "SELECT f.*, c.category_name FROM feedback f LEFT JOIN categories c ON f.category_id = c.category_id";
if ($where) $sql .= ' WHERE ' . implode(' AND ', $where);
$sql .= ' ORDER BY f.submitted_at DESC';

$feedbacks   = $pdo->prepare($sql);
$feedbacks->execute($params);
$feedbacks   = $feedbacks->fetchAll();

$categories  = $pdo->query("SELECT * FROM categories ORDER BY category_name")->fetchAll();

renderHeader('Feedback Management');
renderSidebar('admin', 'Feedback');
?>
<div class="topbar">
  <span class="topbar-title">Feedback Management</span>
</div>

<div class="content">
  <div class="page-header">
    <h1>Feedback</h1>
    <p>Review and manage all submitted feedback.</p>
  </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <?php else: foreach ($feedbacks as $i => $fb): ?>
            <tr>
              <td class="text-muted" style="font-family:'DM Mono',monospace;font-size:12px;"><?= $i+1 ?></td>
              <td><?= sanitize($fb['category_name'] ?? '—') ?></td>
              <td><span class="msg-truncate" style="max-width:260px;"><?= sanitize($fb['message']) ?></span></td>
              <td><?= priorityBadge($fb['priority']) ?></td>
              <td><?= statusBadge($fb['status']) ?></td>
              <td class="text-muted"><?= timeAgo($fb['submitted_at']) ?></td>
              <td>
                <button class="btn btn-outline btn-sm"
                  onclick="openReview(<?= $fb['feedback_id'] ?>, '<?= addslashes(sanitize($fb['message'])) ?>', '<?= $fb['status'] ?>', '<?= $fb['priority'] ?>', '<?= sanitize($fb['category_name'] ?? '') ?>')">
                  Review
                </button>
              </td>
            </tr>
  ...
  <div style="background:#fff;border-radius:12px;width:520px;max-width:90vw;box-shadow:0 20px 60px rgba(0,0,0,0.15);">
    ...
    <div style="padding:20px 24px;">
      <div style="margin-bottom:14px;">
        <div style="font-size:12px;color:var(--text-muted);margin-bottom:4px;">Category / Priority</div>
        <div id="modal-meta" style="font-size:13px;"></div>
      </div>
      <div style="margin-bottom:14px;">
        <div style="font-size:12px;color:var(--text-muted);margin-bottom:4px;">Message</div>
        <div id="modal-msg" style="font-size:13.5px;background:#f9fafb;padding:12px;border-radius:8px;border:1px solid #e5e7eb;"></div>
      </div>
      <form method="POST">
        ...
        <div class="form-group">
          <label class="form-label">Review Notes <span class="text-muted">(optional)</span></label>
          <textarea name="review_notes" class="form-control" rows="3" placeholder="Add notes about this feedback..."></textarea>
        </div>
