...
// Usage: renderSidebar($role, $active)
function renderSidebar($role, $active = '') {
    ...
    if ($role === 'admin') {
        ...
        $nav .= navItem("$base/app/admin/categories.php", svgIcon('tag'), 'Categories', $active);
        ...
        $nav .= navItem("$base/app/user/dashboard.php", svgIcon('grid'), 'Dashboard', $active);
        $nav .= '<span class="nav-section-label">Feedback</span>';
        $nav .= navItem("$base/app/user/submit.php", svgIcon('plus-circle'), 'Submit Feedback', $active);
        $nav .= navItem("$base/app/user/my-feedback.php", svgIcon('message-square'), 'My Feedback', $active);
        $nav .= navItem("$base/app/user/notifications.php", svgIcon('bell'), 'Notifications', $active);
    }
...
function svgIcon($name) {
    $icons = [
        ...
        'tag'            => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z"/></svg>',
